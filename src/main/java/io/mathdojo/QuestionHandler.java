...
public class QuestionHandler extends AzureSpringBootRequestHandler<Question, Question> {
	...
	public Question executeGet(@HttpTrigger(name = "request", methods = {
			HttpMethod.GET }, authLevel = AuthorizationLevel.ANONYMOUS) HttpRequestMessage<Optional<Question>> request,
	...
	public Question executeUpdate(@HttpTrigger(name = "request", methods = {
			HttpMethod.POST }, authLevel = AuthorizationLevel.ANONYMOUS, route = "questions/{questionId}") HttpRequestMessage<Optional<Question>> request,
			ExecutionContext context, @BindingName("questionId") String questionId) {
