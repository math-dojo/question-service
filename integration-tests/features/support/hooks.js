...
BeforeAll({
  ...
  return new Promise((resolve, reject) => {
    ...
    processes.questionservice.functionapp
    ...
    processes.questionservice.functionapp
      .processEventEmitter.stdout.on('data', (data) => {
        if (/Host lock lease acquired by instance ID./.test(data)) {
          ...
          processes.questionservice.functionapp.promiseResolved = true;

          /* Close Process Streams as they are not needed */
          processes.questionservice.functionapp.processEventEmitter.stderr.end();
          processes.questionservice.functionapp.processEventEmitter.stdout.destroy();
          processes.questionservice.functionapp.processEventEmitter.stdin.destroy();

          resolve("Function App Ready!");
        } else if (/((Starting)|(istening))/.test(data)) {
          console.log(`Function App stdout: ${data}`);
        } else if (
          !processes.questionservice.functionapp.promiseResolved &&
          /ERROR\]?/.test(data)
        ) {
          reject(data);
        } else {
  ...
  .catch(err => {
    ...
    const terminationResult = processes.questionservice.functionapp.processEventEmitter.kill();
...
AfterAll(function() {
  ...
  return new Promise((resolve, reject) => {
    console.info(`\nTerminating the functions process running with id ${processes.questionservice.functionapp.processEventEmitter.pid}`);
    resolve(process.kill(processes.questionservice.functionapp.processEventEmitter.pid));
  })
  .finally(() => {
    processes.questionservice.functionapp.processEventEmitter.stderr.end();
    processes.questionservice.functionapp.processEventEmitter.stdout.destroy();
